...
class TAScorePage extends StatefulWidget {
  final int pageSize;

  TAScorePage({Key? key, this.pageSize = 7}) : super(key: key);

  @override
  _TAScorePageState createState() => _TAScorePageState();
}

class _TAScorePageState extends State<TAScorePage> {
  int _page = 1;
  ...
  Future<List<dynamic>> get _evaluations async {
    ...
    Response response = await dio
        .post(UrlConstants.apiUrl + '/evals/notevalpage', data: formData);
  ...
  Future<List> get _list async {
    final response =
        await http.get(Uri.parse(UrlConstants.apiUrl + '/evals/notevalNum'));
