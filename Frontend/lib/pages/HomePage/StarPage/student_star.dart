...
class StudentStarPage extends StatefulWidget {
  final int pageSize;

  StudentStarPage({Key? key, this.pageSize = 7}) : super(key: key);

  @override
  _StudentStarPageState createState() => _StudentStarPageState();
}

class _StudentStarPageState extends State<StudentStarPage> {
  int _page = 1;
  late UserInfo user;
  ...
  Future<List<dynamic>> get _verifiers async {
    try
    {
      ...
      final url = Uri.parse(UrlConstants.apiUrl+'//users/getAreaStarPage');
  ...
  Future<List> get _list async {
    try
    {
      http.Response response = (await http.get(Uri.parse(UrlConstants.apiUrl+'//users/getAreaStarNum/${user.user_province}'), headers: {"Accept": "application/json; charset=utf-8", "Content-Type": "application/json; charset=utf-8"}));
      String str = await response.body.toString();
